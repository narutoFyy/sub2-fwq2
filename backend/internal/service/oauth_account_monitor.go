...
func (s *OAuthAccountMonitorService) checkAccount(ctx context.Context, cfg *OAuthAccountMonitorConfig, account *Account) {
	...
	if s.quotaService != nil {
		...
		if err == nil && usage != nil && usage.RateLimit != nil { quotaStatus = "ok"; if usage.RateLimit.PrimaryWindow != nil { v := 100 - usage.RateLimit.PrimaryWindow.UsedPercent; primary = &v; if v < cfg.QuotaThreshold { quotaLow = true; quotaWindow = "primary" } }; if usage.RateLimit.SecondaryWindow != nil { v := 100 - usage.RateLimit.SecondaryWindow.UsedPercent; secondary = &v; if v < cfg.QuotaThreshold { quotaLow = true; if quotaWindow == "" { quotaWindow = "secondary" } } } } else if err != nil { quotaStatus = "unknown" }
	}
...
func (s *OAuthAccountMonitorService) notify(ctx context.Context, cfg *OAuthAccountMonitorConfig, account *Account, state, previous *OAuthAccountMonitorState) {
	...
	if s.opsRepo != nil { dims := map[string]any{"account_id": account.ID, "account_name": account.Name, "platform": account.Platform, "condition": state.LastConditionKey, "recovery": isRecovery}; event := &OpsAlertEvent{Severity: "warning", Status: OpsAlertStatusFiring, Title: title, Description: body.String(), Dimensions: dims, FiredAt: time.Now().UTC()}; if isRecovery { event.Status = OpsAlertStatusResolved; resolvedAt := event.FiredAt; event.ResolvedAt = &resolvedAt }; _, _ = s.opsRepo.CreateAlertEvent(ctx, event) }
}
